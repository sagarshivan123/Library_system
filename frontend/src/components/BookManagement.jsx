  ...
  return (
    <>
     ...
     <header className='flex flex-col gap-3 md:flex-row md:justify-between md:items-center'>
      <h2 className='text-xl font-medium md:text-2xl md:font-semibold'>{user&&user.role==="Admin"?"Book Management":"Books"}
      </h2>
        <div className='flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4'>
          ...
          className='relative pl-14 w-full sm:w-52 flex gap-4 justify-center items-center py-2 px-4 bg-black text-white rounded-md hover:bg-gray-800'>
               <span className='bg-white flex justify-center items-center overflow-hidden rounded-full w-[25px] h-[25px] text-black text-[27px] absolute left-5'>
                  +
                  </span>
                Add Book
                </button>
            )}
            <input type='text' placeholder='Search books...' className='w-full sm:w-52 border p-2 border-gray-300 rounded-md '
     ...
     {/* Table */}
     {
      ...
      (<div className='mt-6 overflow-auto bg-white rounded-md shadow-lg'><table className='min-w-full border-collapse'>
        <thead>
        ...
        <tbody>
          {
            searchedBooks.map((book,index)=>(
              <tr key={book._id} className={(index+1) % 2===0?
                "bg-gray-50":""
              }>
               ...
               <td className='px-4 py-2 flex space-x-2 my-3 justify-center'>
                <BookA onClick={()=>openReadPopup(book._id)}/>
                  <NotebookPen onClick={()=>openRecordBookPopup(book._id)}/>
                    <Trash2 className='cursor-pointer' onClick={()=>dispatch(toggleDeleteBookPopup(book._id))}/>
               </td>
              )}
              </tr>
            ))}
